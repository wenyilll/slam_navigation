 ...
 */
void PoseEstimator::predict(const double& stamp) {
 ...
 * @param gyro     angular velocity
 */
void PoseEstimator::predict(const double& stamp, const Eigen::Vector3f& acc, const Eigen::Vector3f& gyro) {
  // if (init_stamp.is_zero()) 
  // {
  //   init_stamp = stamp;
  // }
 ...
 */
pcl::PointCloud<PoseEstimator::PointT>::Ptr PoseEstimator::correct(const double& stamp, const pcl::PointCloud<PointT>::ConstPtr& cloud) {
  if (init_stamp==0) {
    init_stamp = stamp;
  }
  ...
  // std::cout << "registration->getFitnessScore()" << registration->getFitnessScore() << std::endl;
  ...
  // std::cout << "points: " << *aligned << std::endl;
    // static int i = 0;
    // std::string file_path = "/home/nvidia/dora_project/indoorslam/localization/hdl_localization/data/aligned" + std::to_string(i) + ".pcd";
    // pcl::io::savePCDFile(file_path, *aligned);
    // i++;
